...
class ActorCritic(nn.Module):
    ...
    def forward(self, state, candidate_actions, action_mask):
        ...
        # 返回概率张量而不是Categorical对象，以兼容DataParallel
    ...
    def _compute_adaptive_weights(self, action_scores, dual_values, state):
        ...
        # 计算评分统计特征，添加数值稳定性
        # 检查输入维度，避免在单个候选动作时计算标准差
        if action_scores.size(1) <= 1:
            # 只有一个或没有候选动作时，使用默认标准差
            action_std = torch.ones(batch_size, 1, device=self.device) * 1e-8
            dual_std = torch.ones(batch_size, 1, device=self.device) * 1e-8
        else:
            # 使用无偏标准差计算，并添加数值稳定性
            action_std = torch.std(action_scores, dim=1, keepdim=True, unbiased=False)  # (batch_size, 1)
            dual_std = torch.std(dual_values, dim=1, keepdim=True, unbiased=False)      # (batch_size, 1)
